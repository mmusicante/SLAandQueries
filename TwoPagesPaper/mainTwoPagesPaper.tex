...
\begin{abstract}
Existing data integration techniques have to be revisited  to query multiple data on the Cloud.
...
Existing data integration techniques have to be revisited to integrate data that are both weakly curated and sometimes described through metadata or schemas. 
...
% which differs from the users’ SLA.
    The question is how can the user efficiently obtain  results for her queries such that they meet her Q\&S requirements, they respect her subscribed contracts with the cloud and they do not neglect services contracts, especially for queries that call several services deployed eventually on different clouds?
...
To our knowledge, projects and deployments of SLAs in the context of Cloud Computing aim and rely on two principles: (i) The negotiation of conditions, which are statically agreed between the parts (\cite{5547150,Dastjerdi:2012:DOA:2275356.2275360,Ortiz:2013:VPS:2486767.2486772} (ii) The monitoring of these conditions during the use of cloud resources in order to detect SLA contracts violation (\cite{6274042,5614035}).


Our work  addresses data integration on a  cloud guided by the SLA exported by different cloud providers and by QoS measures associated to data collections properties: trust, privacy, economic cost. 
This implies several granularities of SLA: first, at the cloud level, the SLA ensured by data providers; then, at the service level, where a service is as  data accessing and processing unit, to be sure that the SLA fits particular service needs; and  at the integration level i.e the possibility to process, correlate and integrate big data collections distributed across different cloud storage supports, providing different quality properties to data (trust, privacy, reliability, etc).
The goal is to propose an SLA guided data integration system exported as a distributed data as a services (DaaS) by a set of cloud providers, that handles the SLA interoperability and collaboration. 
...
Given an expressed query and a set of Q\&S preferences: cost, data provenance, service reputation, execution deadline and so on, the system processes in three steps, as follows: first (i)  SLA derivation computation, performed to filter possible data and services providers using a set of matching algorithms based on  graph structures and RDF specifications.
Then, (ii)  query rewriting for computing possible service compositions giving partial or exhaustive  results according to SLAs content. Finally, (iii)  results  integration into an answer. 
These steps generate intermediate results that are stored as knowledge  to reduce the overhead of further query evaluation processes. Moreover, an integrated SLA is generated to archive the negotiated rules obtained during  the integration. For an incoming query, the whole process is monitored to determine whether the integration SLA is being honoured.
...
Figure~\ref{fig_integ} depicts our extension of the SLA structure that capitalizes on previous SLA negotiation. 
The yellow part of the schema gives an abstract representation of SLA content necessary to describe our extension. 
Compared to standard SLA where two mandatory parties are concerned and a set of optional ones, we propose a set of parties formed by those  actors that are concerned in the data integration process, namely, clouds, services, and user.
Once the filtering process has been completed, the service selects the service composition that will produce the total result set. 
For each selected composition an integrated SLA should be derived from the services and the user SLA. 
Obligations concerning the same items will be confronted to produce some new guaranties specified by the negotiation rules. 
...
The integrated SLA concepts can be resused for evaluating other  queries using  the same concepts. They allow  to use previous compositions that  met the requirements of the user  and an SLA.
\begin{figure}[!t]
\centering
