...
%In our example, non-functional characteristics of the content provision services ({\sf e$_1$, ... e$_4$}) are calculated by services that enforce SLAs  as follows:
%\begin{trivlist}\sf\footnotesize
%\item[~-~e$_i$:]estim-cost$_i$(contents, req\_size, cost, prov\_size, loc).
%QoS preferences$_\mathit{user}$.
%
%                engage$_i$(contents, req\_size, payment)
                %agreedSLA$_i$.
%\end{trivlist}
...
%{QoS preferences}$_\mathit{user}$ expresses the user's preferences with respect to  the way she will receive content from a provider. 
%The requirements will be matched with the agreedSLAs of the services that can provide the content, in order to derive an {\sf agreedSLA}$_i$. 
...
%As usual, SLAs will be represented as conditions over variables that will be used in the rewritten query.
...
%These derived SLA will influence the choice of (data) services that will actually be used for building the result; 
