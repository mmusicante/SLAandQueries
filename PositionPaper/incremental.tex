...
Let us consider the case of a smart city, where particulars can participate on an energy trading pool.
We suppose that this hypothetical city has a large number of households that are capable of selling their energy surplus to other consumers. 
The city has four information hubs, located in four different neighbourhoods, connected to very small energy producers.
Some other producers may be contacted directly, though the services of three different cloud providers.
Each cloud provider exports their own interfaces.
There exist SLAs defined for each individual energy provider server, each hub, and each cloud provider. 

When an on-line consumer requires a list of servers to buy energy from them, a composed web service is generated, in order to fetch each individual or hub service and to put in motion the energy reservation procedures.
Depending on the location of the consumer and producers, different conditions may apply.
Also the cloud providers may publish the conditions for using their services.
All these conditions are expressed as user preferences and SLAs.
Additionally, other non-functional requirements (such as authentication or security requirements) may apply.

In this context, the user supplies an energy demand, his location and payment information and a composite web service should be generated to fulfil the order.
The generated service composition should pick the nearest providers, in accordance to the agreed SLA.
...
The algorithm should be modified to take into account the different SLA.
~\hfill\openbox
\end{example}
...
Let us suppose that the available energy data is needed to produce daily and weekly ``\textit{Star Providers for One MegaWatt-hour}'' producers lists.
