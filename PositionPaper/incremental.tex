...
In this context, the \textit{Global as a View} (\textit{GAV}) methods of query rewriting~\cite{queryRewriting} are suitable.
In the GAV approach, the rewriting process is guided by the specification of both the query to be rewritten and the available databases.
...
The next example shows the main features of the approach proposed in~\cite{UmbertoICWE2013}. 
...
In order to produce a personalised service composition for each user, the algorithm in~\cite{UmbertoICWE2013} takes into account the specification of the composition (which may be produced by the user's browser, including context information).
